<?php
// OTP Authentication API endpoints for AddisTransit
// This file handles phone number authentication with OTP
...
// Centralized DB connection only
require_once __DIR__ . '/db.php'; // provides $pdo using envs or defaults
...
// Send OTP via SMS (Twilio if configured; otherwise fallback to mock)
function sendOTP($phoneNumber, $otp)
{
    ...
    // Normalize FROM to E.164 if provided in local formats like (817) 893-6372
    if ($from && $from[0] !== '+') {
        $digits = preg_replace('/\D/', '', $from);
        if (strlen($digits) === 10) {
            $from = '+1' . $digits; // assume US if 10-digit local
        } elseif (strlen($digits) === 11 && $digits[0] === '1') {
            $from = '+' . $digits;
        } elseif ($digits) {
            $from = '+' . $digits; // best-effort
        }
    }
    ...
    if ($sid && $token && $from) {
        ...
        // Fallback without cURL: use stream context with Basic Auth
    ...
    // Fallback mock if Twilio is not configured
...
{
    // Backward-compatible Ethiopian format normalization
...
// Accept global E.164 phone numbers (e.g., +18178936372) and Ethiopian formats
function normalizePhoneGlobal($phone)
{
    $trimmed = trim($phone);
    // If starts with + and 8-15 digits after, accept as E.164
    if (preg_match('/^\+[1-9]\d{7,14}$/', $trimmed)) {
        return $trimmed;
    }
    // Otherwise, try to normalize as Ethiopian
...
// Handle direct access to auth.php
if (basename($_SERVER['SCRIPT_NAME']) === 'auth.php') {
    // If accessed directly, check if it's a valid auth endpoint
...
if ($requestMethod === 'POST' && (strpos($requestUri, '/auth/send-otp') !== false || $requestUri === '/auth/send-otp')) {
    ...
    try {
        ...
        // Store OTP
        $stmt = $pdo->prepare("INSERT INTO otp_verifications (phone_number, otp_code, expires_at, created_at) VALUES (?, ?, ?, NOW())");
...
elseif ($requestUri === '/api/auth/debug') {
    try {
